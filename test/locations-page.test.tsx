...
test('LocationList renders all locations grouped by country with role labels', () => {
  ...
  expect(screen.getAllByRole('listitem')).toHaveLength(locations.length);
  expect(screen.getAllByText(/Operated|Taught|Proctored/).length).toBeGreaterThanOrEqual(locations.length);
});
